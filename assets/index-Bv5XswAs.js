...
builtins.input = custom_input
            `),await e.yieldToUI(),X=t,$=null,e.finish("載入完成！正在進入主介面..."),t}catch(t){throw $=null,e.interval&&clearInterval(e.interval),console.error("Failed to load Pyodide:",t),t}})(),$)}async function Ze(s,e,t=3e4){let n;const l=new Promise((c,o)=>{n=setTimeout(()=>{o(new Error(`Execution timed out after ${t/1e3} seconds`))},t)});try{return await Promise.race([s.runPythonAsync(e),l])}finally{clearTimeout(n)}}async function et(s){if(s)try{await s.runPythonAsync(`
            import gc
import matplotlib.pyplot as plt
            plt.close('all')
            gc.collect()
...
`)}const q=["sys","os","io","time","timeit","base64","json","datetime","math","re","warnings","builtins","types","random","csv","copy","collections","itertools","functools","pathlib","fractions","struct","operator","string","decimal","abc","enum","typing","textwrap"],w=["numpy","pandas","matplotlib","micropip","js","builtins","QuantLib","mcint","arch","mibian"],N=[...new Set(k.map(h=>{const z=h.trim().split(/\s+/);return z[0]==="from",z[1].split(".")[0]}))].filter(h=>!q.includes(h)&&!w.includes(h)).filter(h=>{var z;return!M.has(h)&&!((z=window.failedPackages)!=null&&z.has(h))});if(N.length===0)return;const D=[],W=[],G=new URL("/pwa_FRM_Book1/",window.location.origin).href,Se=new Set(["scipy","statsmodels","scikit-learn","sympy"]),Q=h=>{if(h){if(typeof h=="string"&&(h.endsWith(".whl")||h.startsWith("http"))){W.push(h);return}Se.has(h)?D.push(h):W.push(h)}},re={sklearn:["scipy","scikit-learn"],"scikit-learn":["scipy"],statsmodels:["scipy"],sympy:[],seaborn:["matplotlib","pandas","scipy","statsmodels"],pandas_datareader:["requests"]};if(N.forEach(h=>{const z=J[h];z?(Q(z.endsWith(".whl")?G+z:z),(me[h]||[]).forEach(j=>{const Y=me[j]||j;Q(Y.endsWith(".whl")?G+Y:Y)})):Q(h),re[h]&&re[h].forEach(S=>{if(!M.has(S)&&!w.includes(S)){const j=J[S]||S;Q(j.endsWith(".whl")?G+j:j)}})}),D.length>0||W.length>0)try{const h=[...new Set(D)],z=[...new Set(W)];p||g(S=>S+`正在動態載入所需套件 [${N.join(", ")}]...
...
await micropip.install(${JSON.stringify(z)}, keep_going=True)
          `)),N.includes("pandas_datareader")&&await s.runPythonAsync(ge),N.includes("matplotlib")&&(await de(s),te||ae("AGG")),N.forEach(S=>{M.add(S);const j=J[S];j&&!j.endsWith(".whl")&&M.add(j)}),p||g(S=>S+`✅ 套件載入完成。
`)}catch(h){if(console.warn("Dependency loading failed:",h),window.failedPackages||(window.failedPackages=new Set),N.forEach(z=>window.failedPackages.add(z)),!p){const z=h.message||String(h);if(z.includes("Can't find a pure Python 3 wheel")){const S=((le=z.match(/for: '([^']+)'/))==null?void 0:le[1])||"unknown";g(j=>j+`⚠️ 套件 "${S}" 無法載入（可能不支援瀏覽器環境），嘗試繼續執行...
