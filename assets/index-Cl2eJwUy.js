...
await micropip.install(${JSON.stringify(w)}, keep_going=True)
          `)),j.includes("pandas_datareader")&&await s.runPythonAsync(me),j.includes("matplotlib")&&(await ie(s),X||ee("AGG")),j.forEach(S=>{$.add(S);const L=K[S];L&&!L.endsWith(".whl")&&$.add(L)}),p||g(S=>S+`✅ 套件載入完成。
